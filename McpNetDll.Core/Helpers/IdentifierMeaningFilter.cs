...
public static class IdentifierMeaningFilter
{
    ...
    private static IEnumerable<string> SplitAllCaps(string token)
    {
        if (token.Length >= 4 && token.All(char.IsUpper))
        {
            ...
            for (var i = 0; i + size <= token.Length; i += size)
    ...
    private static bool ContainsCommonDevSubstrings(string s)
    {
        ...
        return lower.Contains("get") || lower.Contains("set") || lower.Contains("add") || lower.Contains("remove") ||
               lower.Contains("init") || lower.Contains("load") || lower.Contains("save") || lower.Contains("read") ||
               lower.Contains("write") || lower.Contains("name") || lower.Contains("value");
    }
}